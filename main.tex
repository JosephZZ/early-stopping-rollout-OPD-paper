...
\title{Early Stopping Rollout: Early Tokens Are Better For On-Policy Distillation}
...
Such a small change yields such large gains probe us to investigate deeper. We have verified that later position tokens suffer from signal quality issues. But we want to further validate whether the effectiveness of earlier tokens is intrinsically caused by better KL and entropy signals, because we also find that early tokens generally have larger KL and entropy. We test selectors that pick tokens by high KL, high entropy or combined regardless of position; all underperform \method, with most underperforming full-rollout. Therefore, although we don't exclude KL and entropy as auxillary factors, we confirm that the position is a an independent factor rather a saliency proxy. The late position tokens have signal quality issues in themselves. Furthermore, we identified two other mechanisms that may be working together: \textbf{(i)} \emph{Strategy learning}: we find content-wise the early tokens tend to encode strategies and plans, while later tokens tend to focus on specific executions. It may be better to learn just the planning part from the teacher and leave the execution to its own way. \textbf{(ii)} \emph{Convergence Cascade}: after training on the early window, KL divergence on the \emph{untrained} late tokens also drops by 30--40\%; the KL divergence does not have to see late positions to repair them.

These mechanisms explain why \method can beat full-rollout. They do not explain why \method sometimes even \emph{exceeds} the teacher --- a stronger phenomenon we observe on most of our main experiment Table~\ref{tab:main_results}. We trace this to the mode-seeking nature of reverse-KL: the student commits to a suset of supported teacher modes instead of chasing all of them. Our \method loss helps it steer toward a non-dominant but correct mode rather than the teacher's argmax. Empirically, we find \method-trained students commit harder to their own choices than full-rollout students yet agree \emph{less} with the teacher's argmax, and produce trajectories ${\sim}3\times$ shorter than full-rollout and ${\sim}2\times$ shorter than the base student (\S\ref{sec:mode_seek}). It learns the teacher's strategy but preserves its succinct style, leading to better compount behaviors. This finding showcases a potential path of superceding the teacher model in distillation that worth further investigations.
...
% 4. MAIN RESULTS
% ============================================================================
\section{Main Results: \Method{} Dominates Full-Sequence}
...
\subsection{Results Across Families, Tasks, Regimes, and Scales}
\label{sec:main_results_sub}

Table~\ref{tab:main_results} presents the unified main results across tasks, model families, scales, and training regimes. We report the best performance achieved across all training steps for each configuration. But notice that full rollout distillation frequently collapse to much lower performance as it trains. We denote the sessions that collapses with a down arrows.
...
\multirow{4}{*}{Qwen2.5-Math 1.5B $\to$ Qwen3 1.7B} & Student baseline & 50.95 & 72.80 & 31.10 & 26.20 & 2.70 \\
 ...
 & \textbf{Full Rollout} & 62.35$^{\downarrow}$ & 74.60$^{\downarrow}$ & 40.20$^{\downarrow}$ & 35.40$^{\downarrow}$ & 58.20 \\
...
\multirow{4}{*}{Gemma-2 2B $\to$ Gemma-3 4B} & Student baseline & 13.45 & 28.20 & 23.78 & 20.10 & 73.17 \\
 ...
 & \textbf{Full Rollout} & 22.95 & 31.40 & 22.00$^{\downarrow}$ & 17.70$^{\downarrow}$ & 76.83 \\
 & \textbf{\Method{}} & \textbf{27.20} & \textbf{39.40} & \textbf{28.70}\,\beatsteacher & \textbf{24.40}\,\beatsteacher & \textbf{79.00}\,\beatsteacher \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{\boldmath\methodfull beats full rollout across model families and tasks, and frequently surpasses teacher performance.} Across the cells of Table~\ref{tab:main_results}, \method{} beats full rollout on essentially every cell, and in six cells \method{} \emph{surpasses the teacher} outright (Qwen math avg@4, pass@4; Qwen HumanEval and HumanEval+; Qwen BFCL full\_acc; Gemma coding on all four columns; Gemma BFCL full\_acc). The Gemma coding story is the sharpest: full rollout drives HumanEval pass@1 to $22.00$, \emph{below} the $23.78$ student baseline and worse than the $20.70$ teacher, while \method{} recovers $31.10$. Qwen-math full rollout is marked $^{\downarrow}$ because its best number ($62.35$ avg@4) is a non-stable best step: under $n{=}16$ it collapses via boxed-repetition to $46.3\%$ by step~100 (Appendix~\ref{app:fullseq_degradation}).

\paragraph{\boldmath\methodfull matches or beats full rollout across full fine-tuning and LoRA settings.} The \method{} story is not a LoRA artifact. Table~\ref{tab:fullft_math} reports FullFT on MATH-500 for both Qwen and Gemma. On Qwen the gap narrows (\textbf{Full Rollout} $58.20$ vs \method{} $56.20$ avg@4; both well below the LoRA-\method{} best of $65.85$). On Gemma \method{} still dominates: \method{} beats \textbf{Full Rollout} by $+12.75$pp avg@4 and $+15.40$pp pass@4. \Method{} is the safer choice in both parameter regimes.

\paragraph{\boldmath\methodfull beats full rollout across model sizes.} The advantage persists as both teacher and student grow. Table~\ref{tab:scaling_math} reports three scaling regimes. With the Qwen3-4B teacher, \textbf{Full Rollout} peaks at step~50 ($67.45$) and then \emph{collapses} via boxed-repetition to $55.05$ by step~200 --- a $-12.4$pp drop across training --- while \method{} stays stable and climbs to $68.95$. Against the 8B teacher, \method{} still edges \textbf{Full Rollout} ($67.85$ vs $66.75$); \textbf{Full Rollout} was only run at step~50, so no trajectory is available. The pattern persists at much larger scale: with a Qwen2.5-Math-7B student and Qwen3-14B teacher, \textbf{Full Rollout} again peaks at step~50 ($68.85$) and drifts to $62.40$ by step~200, while \method{} holds across all checkpoints.
...
\paragraph{\boldmath\methodfull is more robust than full rollout in training.} Across our (pair $\times$ task $\times$ regime) matrix, full-rollout distillation degrades with training in seven cells; \method{} degrades nowhere. Appendix Table~\ref{tab:app_fullseq_collapse} reports each collapsing cell as a (peak $\rightarrow$ lowest) pair with the corresponding step indices. The headline failures are: Qwen-LoRA coding falls from $40.2$ at s50 to $26.8$ by s350 ($\Delta = -13.4$pp); Qwen-LoRA math under $n_{\text{samples}}{=}16$ collapses via boxed-repetition from $65.60$ at s50 to $43.80$ by s200 ($\Delta = -21.80$pp, ending below the $50.95\%$ student baseline); Qwen-LoRA math with the Qwen3-4B teacher peaks at $67.45$ at s50 and drifts to $54.45$ by s100 ($\Delta = -13.00$pp); and Gemma-LoRA coding peaks at $22.0$ at s50 and falls to $11.6$ ($\Delta = -10.4$pp), more than $12$pp below the student baseline. Three of seven full-rollout collapses end below the student baseline. \Method{} on the same cells stays within $\pm 1$pp of its peak across training (Tables~\ref{tab:app_math_main},~\ref{tab:app_coding_lora}).

\paragraph{\boldmath\methodfull is not sensitive to the choice of $N$.} Figure~\ref{fig:n_sweep} sweeps $N$ on MATH-500 and reveals a robust sweet spot: all rollout cutoffs from $N{=}50$ upward converge to $\sim$66\% avg@4, outperforming full-sequence distillation ($62.35\%$) by $3$--$4$pp. The method is not sensitive to the exact choice of $N$ within this plateau, so no per-task tuning is required.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/fig_n_sweep.png}
    \caption{\textbf{Rollout Length $N$ Sweep on MATH-500.} LoRA, Qwen2.5-Math-1.5B $\rightarrow$ Qwen3-1.7B, $n{=}1$, $3{,}200$ problems; best avg@4 across training steps. Full-seq and the undistilled baseline are shown as horizontal references. Performance saturates for $N \in [50, 200]$ and all points in this range beat full-seq.}
    \label{fig:n_sweep}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{\textbf{Training efficiency.} Single A6000 (48\,GB), bs${=}16$, $n{=}1$, teacher Qwen3-1.7B. Per-phase wall time and peak GPU memory share the same phases. Phases run sequentially; the memory ``Total'' columns span both rows because they describe the co-resident footprint, i.e., the sum of per-phase peaks if teacher and student stay resident simultaneously instead of being swapped in/out.}
\label{tab:timing}
\end{table}

Table~\ref{tab:timing} shows that \method{} achieves a 24$\times$ wall-clock speedup and reduces peak training memory by $\sim 4\times$. The dominant cost in full-seq is autoregressive generation ($180$\,s/step for sequences averaging ${\sim}1000$ tokens); \method{} generates only $N{=}100$ tokens (5\,s/step). \Method{} also does not require a vLLM/SGLang inference server---100 tokens can be generated via standard HF generate. Full-seq with an 8B teacher OOMs on a 48\,GB A6000; \method{} runs comfortably, unlocking larger teachers on smaller GPUs.

% ============================================================================
% 6. MECHANISMS (Part 3 of the deck)
% ============================================================================
\section{Why Does \Method{} Work? }
\label{sec:mechanisms_section}

The result that a one-line change matches or exceeds full-sequence distillation --- at a fraction of the compute, with stability everywhere the full-sequence baseline degrades --- is surprising enough that it demands a deeper mechanism. We structure this section around three questions: \textbf{(\S\ref{sec:token_selection})} Is the early-position effect simply a proxy for high-KL or high-entropy tokens? \textbf{(\S\ref{sec:mechanisms})} If not, what additional structural reasons make the early window privileged? \textbf{(\S\ref{sec:mode_seek})} Why does \method{} sometimes \emph{exceed} the teacher?

\subsection{Is the Effectiveness of the Early Position Caused by KL and Entropy?}
\label{sec:token_selection}

Figure~\ref{fig:three_curves} shows that early positions are simultaneously high-KL and high-entropy, so a natural alternative to positional selection is \emph{information-theoretic} selection: pick the $k$ tokens with the highest KL divergence or highest student/teacher entropy, regardless of position. We tested every such selector (Table~\ref{tab:token_select}): all underperform \method{}, and most underperform full-sequence. For each strategy in Table~\ref{tab:token_select} we keep the full 400-token on-policy rollout, compute the selector, and apply the reverse-KL loss only to the selected tokens.
...
Two findings fall out of Table~\ref{tab:token_select}. \textbf{(i)} Top-$\mathrm{RKL}{=}100$, the selector that captures the most raw KL signal, is the \emph{worst} (53.35\%) --- worse than no distillation. Increasing $k$ to 200 only partly recovers. \textbf{(ii)} Every KL/entropy/combined selector underperforms full-sequence (62.35\%), which in turn underperforms \method{} (65.85\%). These selectors all concentrate on the contaminated late region where the teacher is scoring its own continuation of the student's previously generated tokens; the benefit of the early region is therefore \emph{structural}, not reducible to any scalar per-token statistic.

\paragraph{Takeaway.} \textbf{\Method{} is not mainly caused by KL or entropy.} Position is an independent, load-bearing axis --- not ``high-KL in disguise'' and not ``high-entropy in disguise''. Whatever makes the early window privileged is structural; we identify two such reasons next.

\subsection{Strategy Learning and Convergence Cascade}
\label{sec:mechanisms}

Section~\ref{sec:token_selection} rules out scalar per-token saliency. Beyond the late-position contamination already established in \S\ref{sec:motivation}, we identify two further structural mechanisms that jointly explain why training on the early window is so effective.

\paragraph{Strategy learning --- the early window is where planning tokens live.} The early-position KL spike concentrates on planning / structural tokens: the highest-KL tokens in our corpus are ``To'' (KL 6.90, $n{=}8806$), ``First'' (9.98), ``Let'', ``The'', ``Therefore'' (7.95), ``Thus'', along with format-opening tokens \texttt{\textbackslash[}, \texttt{\textbackslash(}, and code-fence \verb|```| (see Table~\ref{tab:app_high_kl_tokens}). These tokens \emph{select} the reasoning trajectory --- different openings commit the student to algebra-with-variables, factoring, or numeric-estimation paths on the same problem. The teacher's conditional distribution at these tokens is genuinely multi-modal (entropy 1.07), so reverse-KL has room to move the student to a single teacher-supported mode. After the plan is committed, the remainder is execution: the teacher is near-deterministic (entropy 0.14) and there is little signal to extract. \emph{Teach the plan; the execution follows} --- and no scalar saliency can recover the early-window region because what matters is where in the rollout a token lives, not how surprising it is locally.

\paragraph{Convergence cascade --- training the early window lowers KL everywhere.} A natural worry is that \method{} helps only inside the trained window. The opposite holds. After \method-200 training, per-position KL between distilled student and teacher drops by $50$--$60\%$ inside $[0, N]$ \emph{and} by $30$--$40\%$ outside it (Figure~\ref{fig:cascade}). Comparing distilled vs.\ base responses on the same prompts, late tokens --- which receive zero direct supervision --- change as much as early tokens in surface content (Jaccard to baseline $=0.251$ for both halves). Autoregressive generation explains the cascade: once the student has committed to the right plan in the first $N$ tokens, the remaining rollout is a lower-KL, more on-policy completion for the teacher, so no gradient is needed there. The loss does not have to \emph{see} late positions to align them.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/kl_cascade_curves.png}
    \caption{\textbf{The convergence cascade.} Per-position KL between distilled student and teacher, before vs.\ after \method-$200$ training. Yellow band: positions $[0, 200]$ that actually receive training loss. Blue band: the KL gap closed by training. Positions $200+$ --- which see no direct training signal --- drop to the same KL as the trained region, confirming that alignment on the early window cascades through the autoregressive rollout.}
    \label{fig:cascade}
\end{figure}

\subsection{Why Does \Method{} Sometimes Exceed the Teacher?}
\label{sec:mode_seek}
